...
export const TaskCard = ({ task, onToggleComplete, onEdit, onDelete }) => {
  ...
  return (
    <div className="card-depth bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 p-5 hover:border-emerald-200 hover:-translate-y-0.5 group">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3
            className={`font-semibold text-gray-900 dark:text-slate-100 break-words ${
              isCompleted ? 'line-through text-gray-400 dark:text-slate-600' : ''
            }`}
          >
            {task.title}
          </h3>
          {task.description && (
            <p className="text-sm text-gray-500 dark:text-slate-500 mt-1 break-words">
              {task.description}
            </p>
          )}
            ...
            {task.priority && (
              <span
                className={`px-2 py-0.5 rounded-full font-medium ${
                  priorityColor[task.priority] || 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300'
                }`}
              >
                {task.priority}
              </span>
